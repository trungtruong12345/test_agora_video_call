...
;(async () => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    await page.waitForSelector(
      `.grid-template-columns-auto-10.d-grid.grid-row-gap-10`
    )
    await page.waitForSelector(`.w-300p.p-12p.font-size-21[type="submit"]`)
    const iconElement = await page.waitForSelector(
      `.grid-template-columns-auto-10.d-grid.grid-row-gap-10 > div:nth-of-type(${iconIndex})`
    )
    ...
    if (!isChecked) {
      await page.click(
        `.p-application_form__content.p-0.m-0.border-unset.bg-white.d-flex.flex-column`
      )
    }
    ...
    pages.push(page)
    page.evaluate(() => {
      console.log('init onbeforeunload');
      try {
  ...
  setInterval(async () => {
    ...
    page.evaluate(() => {
      try {
        ...
        const min = 1
        const max = 6
        const randomReactionIndex =
          Math.floor(Math.random() * (max - min + 1)) + min
        $(
          `.comments-screen-zone button:nth-of-type(${randomReactionIndex})`
        ).click()
        return true
      } catch (error) {
        console.log(error)
